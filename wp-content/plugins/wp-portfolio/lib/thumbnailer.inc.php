 ...
 */
function WPPortfolio_generateFilename($imageurl, $args)
{
	return sprintf('%s_%s.jpg', md5($imageurl), md5(serialize($args)));
}
...
function WPPortfolio_getAdjustedCustomThumbnail($imageurl, $size_override = false, $forceUpdate = false) 
{
    ...
    $filename = WPPortfolio_generateFilename($imageurl, $args);    
...
{ 
	$proAccount = WPPortfolio_isPaidAccount();
	...
	// Use the javascript mode for free basic users
	else 
	{		
		// Handle custom sizes
	    if ($size_override) {
    		$stwsize = $size_override;
	    } else { 
	    	$stwsize = stripslashes(get_option('WPPortfolio_setting_stw_thumb_size'));
	    }
	    
	    // Access key (not secret key) needed for JS.
	    $stwaccesskeyid = stripslashes(get_option('WPPortfolio_setting_stw_access_key'));				
		$imagetag = sprintf('<script type="text/javascript">stw_pagepix("%s", "%s", "%s");</script>', $url, $stwaccesskeyid, $stwsize);
	}
							...
							$args = null, $forceUpdate = false, $fetchMethod = "curl")
{
    ...
    // Use arguments to work out the target filename
    $filename = WPPortfolio_generateFilename($url, $args);    
	$filepath = $actualThumbPath . $filename;
	...
	// File downloaded successfully
	if (($usingCache || (!$usingCache && $jpgurl)) && file_exists($filepath)) {
...
function WPPortfolio_checkWebsiteThumbnailCaptured($url, $args = null, $fetchMethod = "curl")
{
	...
	WPPortfolio_debugLogThumbnailRequest($url, 'web', ($imageURL != false), $detail, $args, $resultData['status']);
...
function WPPortfolio_xml_processReturnData($data)
{
	...
	// Thumbnail queued
	else if ($stw_response_status == 'Success' && ($stw_action == 'noexist' || $stw_action == 'queued'))
	{
