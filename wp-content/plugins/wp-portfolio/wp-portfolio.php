...
define('WPP_DEFAULT_PAGING_TEMPLATE_PREVIOUS', 	'Previous');
define('WPP_DEFAULT_PAGING_TEMPLATE_NEXT', 		'Next');
...
function WPPortfolio_menu()
{
	...
	add_submenu_page(__FILE__, 'WP Portfolio - Error Logs', 		'Error Logs'.$errorCountMsg, 'manage_options', 'WPP_show_error_page', 'WPPortfolio_showErrorPage');
}
...
{	
	...
	if (is_admin())
	{
		WPPortfolio_install_forceTableUpgrade();
...
/**
 * Force a table upgrade if requested by user.
 * /wp-admin/?upgrade_tables=true
 */
function WPPortfolio_install_forceTableUpgrade()
{
	if (isset($_GET['upgrade_tables']) && $_GET['upgrade_tables'] == 'true' && current_user_can('manage_options')) {
		printf('Upgrading WP Portfolio Tables...<br/>');
		flush();
		WPPortfolio_install_upgradeTables(true, true);
		printf('Done.<br/>');
		exit();
	}
}
...
function WPPortfolio_messages()
{
	...
	if ($accountType != 'free' && $accountType != 'paid') {
		WPPortfolio_showMessage('WP Portfolio has been upgraded, and there\'s been a slight settings change. Please choose your Shrink The Web account type in the <a href="'.WPP_SETTINGS.'#stw-account">Portfolio Settings</a>.', true);
	}
}
...
<?php 	
	...
	else if (WPPortfolio_getArrayValue($_POST, 'update') == 'codepage_upgrade')
	{
		...
		WPPortfolio_showMessage("WP Portfolio tables have successfully been upgraded to UTF-8.");
	}
	...
	{		
		...
		if ($newCacheLoc && $newCacheLoc != $oldCacheLoc)
		{
			...
			WPPortfolio_showMessage(sprintf(__("The cache location has successfully been changed. The new cache location is now:<br/><br/><tt>%s</tt>"), $newURLPath));		
		}
	...
	$formElem->description = "The <a href=\"".WPP_DOCUMENTATION."#doc-stw\">Shrink The Web</a> Secret Key is around 5-10 characters. This key is never shared,<br>it is only stored in your settings and used to generate thumbnails for this website."
							 ."<a name=\"stw-account\"></a>"; // The anchor for the option below
	$form->addFormElement($formElem);
	
	
	$formElem = new FormElement("setting_stw_account_type", "STW Account Type");
	$formElem->value = $settings['setting_stw_account_type'];
	$formElem->setTypeAsComboBox(array('' => '-- Select an account type --', 'free' => 'Free Account', 'paid' => 'Paid (Basic or Plus) Account'));
	$formElem->description = '&bull; The type of account you have with <a href="' . WPP_DOCUMENTATION . '#doc-stw">Shrink The Web</a>. Either a <i>free  account</i>, or a <i>paid (basic or plus) account</i>. Your account type determines how the portfolio works.<br/>'
						 . '&bull; Learn more about account types in the <a href=http://wordpress.org/extend/plugins/wp-portfolio/faq/" target="_new">WP Portfolio FAQ section.</a>';
	$form->addFormElement($formElem);
	
	$form->addBreak('wpp-thumbnails', '<div class="wpp-settings-div">Thumbnail Settings</div>');
	
	
	// Thumbnail sizes - Paid Only
	if (WPPortfolio_isPaidAccount())
	{
		$formElem = new FormElement("setting_stw_thumb_size_type", "What thumbnail sizes do you want to use?");
		$formElem->value = $settings['setting_stw_thumb_size_type'];
		$formElem->setTypeAsComboBox(array('standard' => 'Standard STW Sizes', 'custom' => 'My own custom sizes'));
		$formElem->cssclass = 'wpp-size-type';
		$form->addFormElement($formElem);
		
		$formElem = new FormElement("setting_stw_thumb_size_custom", "Custom Thumbnail Size (Width)");
		$formElem->value = $settings['setting_stw_thumb_size_custom'];
		$formElem->cssclass = 'wpp-size-custom';
		$formElem->description = '&bull; Specify your desired width for the custom thumbnail. STW will resize the thumbnail to be in a 4:3 ratio.<br/>' . 
								 '&bull; This feature requires a STW Paid (Basic or Plus) account with custom thumbnail support.';
	...
	$form->addBreak('wpp-thumbnails', '<div class="wpp-settings-div">Miscellaneous Settings</div>');
	...
	$formElem->description = "Enables logging of successful thumbnail requests too (all errors are logged regardless).";
	$form->addFormElement($formElem);		
	...
	<table id="wpp-checklist" cellspacing="0" cellpadding="0">
		<tbody>
			<tr>
				...
				<td>
                    ...
                    <?php else : ?>        
						...
						<span class="wpp-error-info"><?php _e("WP Portfolio requires PHP 5 or above.", "wp-portfolio"); ?></span>
					<?php endif; ?>
				</td>
			</tr>	
			...
			<tr>
				...
				<?php else : ?>
					...
					<td>        
						<img src="<?php echo WPPortfolio_getPluginPath(); ?>/imgs/icon_stop.png" alt="No" />
						<span class="wpp-error-info"><?php _e("WP Portfolio requires a directory for the cache that's writeable.", "wp-portfolio"); ?></span>
					</td>
				<?php endif; ?>
			</tr>	
			...
			<tr>
				...
				?>
				<td><?php _e("open_basedir Restriction", "wp-portfolio");?></td>
	...
	<p>You can either have the thumbnail cache stored in the <b>plugin directory</b> (which gets deleted when you upgrade the plugin), or you can have the thumbnail cache stored in the <b>wp-content directory</b> (which doesn't get deleted when you upgrade wp-portfolio). This is only useful if your thumbnails are set to never be updated and you don't want to lose the cached thumbnails.</p>
	<dl>
		<dt><?php _e('Plugin Location', 'wp-portfolio'); ?>: <?php if (WPPortfolio_getCacheSetting() == 'plugin') { printf('&nbsp;&nbsp;<i class="wpp-cache-selected">(%s)</i>', __('Currently Selected')); } ?></dt>
		<dd><tt><?php echo WPPortfolio_getThumbPathURL('plugin'); ?></tt></dd>	
		
		<dt><?php _e('wp-content Location', 'wp-portfolio'); ?>: <?php if (WPPortfolio_getCacheSetting() == 'wpcontent') { printf('&nbsp;&nbsp;<i class="wpp-cache-selected">(%s)</i>', __('Currently Selected')); } ?></dt>
		<dd><tt><?php echo WPPortfolio_getThumbPathURL('wpcontent'); ?></tt></dd>
	</dl>
	...
	<p>As of V1.18, WP Portfolio uses UTF-8 as the default codepage for all text fields. Previously, for non Latin-based languages, the lack of UTF-8 support caused rendering issues with characters (such as using question marks and blocks for certain characters).
	To upgrade to the new UTF-8 support, just click the button below. If you're <b>not experiencing problems</b> with website names and descriptions, then there's no need to click this button.</p>
	<?php
	$form = new FormBuilder(__('codepage_upgrade', 'wp-portfolio'));
...
function WPPortfolio_showDocumentationPage() 
{
	...
	<p>If you have any comments, ideas or any other feedback on this plugin, please leave comments on the <a href="http://wordpress.org/tags/wp-portfolio?forum_id=10">WP Portfolio Support Forum on Wordpress.org</a>.</p>
	...
	<p>The plugin requires you to have a free (or paid) account with <a href="http://www.shrinktheweb.com" target="_blank">Shrink The Web (STW)</a> if you wish to generate the thumbnails <b>dynamically</b>. Please read <a href="http://wordpress.org/extend/plugins/wp-portfolio/faq/" target="_blank">the first FAQ about account types</a> to learn more. If you have a paid account with STW, this plugin will automatically handle the caching of thumbnails to give your website fast loading times.</p>

	<p>However, you do not need an account with ShrinkTheWeb to use this plugin if you capture screenshots of your websites yourself. Just can capture your own screenshots as images, upload those images to your website, and then link to them in the <b>Custom Thumbnail URL</b> field.</p>
	...
	<ul style="margin-left: 30px; list-style-type: disc;">
		...
		<li><strong><?php echo WPP_STR_WEBSITE_CUSTOM_FIELD; ?></strong> - Replace with the custom field data.</li>
	</ul>
	...
	<table class="form-table">
		...
		<tr class="form-field">
			<th scope="row"><label for="default_template_css_widget">Widget CSS</label></th>
			<td>
	...
	<a name="doc-paging"></a>
	<h2>Portfolio Paging Templates</h2>	
	
	<p>The default templates specifically for the paging of websites (when there are more websites that you want to fit on a single page).</p>
	<ul style="margin-left: 30px; list-style-type: disc;">
		<li><strong>%PAGING_PAGE_CURRENT%</strong> - Replace with the current page number.</li>
		<li><strong>%PAGING_PAGE_TOTAL%</strong> - Replace with the total number of pages.</li>
		<li><strong>%PAGING_ITEM_START%</strong> - Replace with the start of the range of websites/thumbnails being shown on a particular page.</li>
		<li><strong>%PAGING_ITEM_END%</strong> - Replace with the end of the range of websites/thumbnails being shown on a particular page.</li>
		<li><strong>%PAGING_ITEM_TOTAL%</strong> - Replace with the total number of websites/thumbnails in the portfolio.</li>
		<li><strong>%LINK_PREVIOUS%</strong> - Replace with the link to the previous page.</li>
		<li><strong>%LINK_NEXT%</strong> - Replace with the link to the next page.</li>
		<li><strong>%PAGE_NUMBERS%</strong> - Replace with the list of pages, with each number being a link.</li>
	</ul>
	
	<form>
	<table class="form-table">
		<tr class="form-field">
			<th scope="row"><label for="default_template_paging">Paging Template</label></th>
			<td>
...
?&gt;
	...
	<h2>Advanced Features - Disabling pagepix.js inclusion.</h2>
	<p>If you're not using STW at all, and you want to completely disable <b>pagepix.js</b> from appearing in the header, then copy and paste the following code into your theme's <b>functions.php</b>.</p>
	<code>define('WPPORTFOLIO_NO_PAGEPIX', true);</code>
...
<?php 	
	...
	$form->addBreak('settings_paging', '<div class="settings-spacer">&nbsp;</div><h2>Portfolio Paging Settings</h2>');
	$formElem = new FormElement("setting_template_paging", "Paging HTML Template");				
	$formElem->value = htmlentities($settings['setting_template_paging']);
	$formElem->description = '&bull; This is the template used to render the paging for the thumbnails.<br/>
							  &bull; A complete list of tags is available in the <a href="'.WPP_DOCUMENTATION.'#doc-paging">Portfolio Paging Templates</a> section in the documentation.';
	$formElem->setTypeAsTextArea(3, 70); 
	$form->addFormElement($formElem);	
	
	$formElem = new FormElement("setting_template_paging_previous", "Text for 'Previous' link");				
	$formElem->value = htmlentities($settings['setting_template_paging_previous']);
	$formElem->description = 'The text to use for the \'Previous\' page link used in the thumbnail paging.'; 
	$form->addFormElement($formElem);
	
	$formElem = new FormElement("setting_template_paging_next", "Text for 'Next' link");				
	$formElem->value = htmlentities($settings['setting_template_paging_next']);
	$formElem->description = 'The text to use for the \'Next\' page link used in the thumbnail paging.'; 
	$form->addFormElement($formElem);
	
	
	$form->addBreak('settings_css', '<div class="settings-spacer">&nbsp;</div><h2>Portfolio Stylesheet (CSS) Settings</h2>');
	
	// Enable/Disable CSS mode
	$formElem = new FormElement("setting_disable_plugin_css", "Disable Plugin CSS");
	$formElem->value = $settings['setting_disable_plugin_css'];
	$formElem->setTypeAsCheckbox("If ticked, don't use the WP Portfolio CSS below.");
	$formElem->description = "&bull; Allows you to switch off the default CSS so that you can use CSS in your template CSS file.<br/>
							  &bull; <strong>Advanced Tip:</strong> Once you're happy with the styles, you should really move all the CSS below into your template <tt>style.css</tt>. This is so that visitor browsers can cache the stylesheet and reduce loading times. Any CSS placed here will 
	...
	$formElem->description = 'This is the CSS code used to style the portfolio.';
	...
	$formElem->description = 'This is the CSS code used to style the paging area if you are showing your portfolio on several pages.';
	$formElem->setTypeAsTextArea(6, 70); 
	$form->addFormElement($formElem);	
	
	$formElem = new FormElement("setting_template_css_widget", "Widget CSS");				
	$formElem->value = htmlentities($settings['setting_template_css_widget']);
	$formElem->description = 'This is the CSS code used to style the websites in the widget area.';
	$formElem->setTypeAsTextArea(6, 70); 
	$form->addFormElement($formElem);
...
<?php 		
	...
	else if ($siteid > 0 && isset($_GET['duplicate']))
	{
		...
		if ($websitedetails)
		{
			...
			$editOriginal	= sprintf('<a href="'.WPP_MODIFY_WEBSITE.'&editmode=edit&siteid=%s" title="Edit \'%s\'">%s</a>', $siteidOriginal, $nameOriginal, $nameOriginal);
			$editNew   		= sprintf('<a href="'.WPP_MODIFY_WEBSITE.'&editmode=edit&siteid=%s" title="Edit \'%s\'">%s</a>', $siteidNew, $websitedetails['sitename'], $websitedetails['sitename']);
			
			$message = sprintf('The website \'%s\' was successfully copied to \'%s\'.',$editOriginal, $editNew);
			WPPortfolio_showMessage($message);
		}
	}
	...
	if ($websites)
	{
		...
		$column = new TableColumn("Custom Info", "custominfo");
		...
		foreach ($websites as $websitedetails)
		{
			...
			$editClickable    = '<a href="'.WPP_MODIFY_WEBSITE.'&editmode=edit&siteid='.$websitedetails->siteid.'" title="Edit \''.stripslashes($websitedetails->sitename).'\'" class="wpp-edit">';
			...
			if ($websitedetails->customthumb)
			{
				...
				$customThumb = '<a href="'.$websitedetails->customthumb.'" target="_new" title="Open custom thumbnail in a new window">View Image</a>';
			} 
			...
			if ($customThumb) {
				$rowdata['custominfo']		= sprintf('<span class="wpp-custom-thumb"><b>Custom Thumb:</b><br/>%s</span>', $customThumb);
			}
			
			if ($websitedetails->customfield) {
				$rowdata['custominfo']		.= sprintf('<span class="wpp-custom-field"><b>Custom Field:</b><br/>%s</span>', $websitedetails->customfield);
			}
			...
			$rowdata['siteactive']  = ($websitedetails->siteactive ? 'Yes' : '<b>No</b>'); 
			...
			// Refresh link - only for paid accounts or custom thumbnails			 
			if ($paidAccount || $websitedetails->customthumb) {
				$refreshAction = '&bull; <a href="'.WPP_WEBSITE_SUMMARY.'&refresh=yes&siteid='.$websitedetails->siteid.'" class="wpp-refresh" title="Force a refresh of the thumbnail">Refresh</a>';
			} else {
				$refreshAction = '<span class="wpp-refresh-disabled">&bull; Refresh</span>';
			}
			
			
			// The various actions - Delete | Duplicate | Edit
			$rowdata['action'] 		= $refreshAction . '<br/>' .
									  '&bull; '.$editClickable.'Edit</a><br/>' . 
									  '&bull; <a href="'.WPP_WEBSITE_SUMMARY.'&duplicate=yes&siteid='.$websitedetails->siteid.'" title="Duplicate this website">Duplicate</a><br/>' .
									  '&bull; <a href="'.WPP_WEBSITE_SUMMARY.'&delete=yes&siteid='.$websitedetails->siteid.'" title="Delete this website...">Delete</a><br/>' 
									  ; 
			;
...
function WPPortfolio_showErrorPage() 
{
	...
	if (isset($_POST['wpp-clear-logs']))
	{
		...
		WPPortfolio_showMessage("Debug logs have successfully been emptied.");
	}
	...
	<h2>Error Log</h2>
		...
		<form class="wpp-button-right" method="post" action="<?= str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
			<input type="submit" name="wpp-refresh-logs" value="Refresh Logs" class="button-primary" />
			<input type="submit" name="wpp-clear-logs" value="Clear Logs" class="button-secondary" />
	...
	<?php 
		...
		if ($logMsgs)
		{
			printf('<div id="wpp_error_count">Showing a total of <b>%d</b> log messages.</div>', $wpdb->num_rows); 
			?>
...
function WPPortfolio_modify_website()
{
	...
	if ( isset($_POST) && isset($_POST['update']) )
	{
		...
		if (strlen($data['customfield']) > 255) {
			array_push($errors, "Sorry, but the custom field is limited to a maximum of 255 characters.");
		}
		...
		if ($data['siteadded']) 
		{
			...
			// Invalid date
			if ($dateTS == 0) {
				array_push($errors, "Sorry, but the 'Date Added' date format was not recognised. Please enter a date in the format <em>'yyyy-mm-dd hh:mm:ss'</em>.");
			}
	...
	$form->addBreak('advanced-options', '<div id="wpp-hide-show-advanced" class="wpp_hide"><a href="#">Show Advanced Settings</a></div>');
	...
	$formElem = new FormElement("siteadded", "Date Website Added");				
	$formElem->value = WPPortfolio_getArrayValue($websitedetails, 'siteadded');
	$formElem->description = "Here you can adjust the date in which the website was added to the portfolio. This is useful if you're adding items retrospectively. (valid format is yyyy-mm-dd hh:mm:ss)";
	$form->addFormElement($formElem);
	...
	$formElem = new FormElement("website_customfield", "Custom Field<br/><span class=\"wpp-advanced-feature\">&bull; Advanced Feature</span>");				
	$formElem->value = WPPortfolio_getArrayValue($websitedetails, 'customfield');
	$formElem->cssclass = "long-text";
	$formElem->description = "Allows you to specify a value that is substituted into the <tt><b>".WPP_STR_WEBSITE_CUSTOM_FIELD."</b></tt> field. This can be any value. Examples of what you could use the custom field for include:
								<br/>&bull; Affiliate URLs for the actual URL that visitors click on.
								<br/>&bull; Information as to the type of work a website relates to (e.g. design work, SEO, web development).
							  ";
	$form->addFormElement($formElem);

	
	// Advanced Features
	$formElem = new FormElement("website_specificpage", "Use Specific Page Capture<br/><span class=\"wpp-advanced-feature\">&bull; Advanced Feature</span><br/><span class=\"wpp-stw-paid\">&bull; STW Paid Account Only</span>");
	...
	$formElem->description = "&bull; <b>Requires Shrink The Web 'Specific Page Capture' Paid (Basic or Plus) feature.</b><br/>
 ...
 */
function WPPortfolio_install_upgradeTables($upgradeNow, $showErrors = false)
{
	global $wpdb;
...
function WPPortfolio_scripts_Backend()
{
	...
	// For free users
	if (!WPPortfolio_isPaidAccount()) {
		wp_enqueue_script('stw-pagepix', 'http://www.shrinktheweb.com/scripts/pagepix.js', false, time());
	}
}
...
{		
	// Force abort if defined no PAGEPIX.
	if (defined('WPPORTFOLIO_NO_PAGEPIX')) {
		return;
	}
...
function WPPortfolio_handleForcedThumbnailRefresh() 
{
	...
	{
		// If we have a free account, then we shouldn't get a request to refresh the thumbnail.
		if (!WPPortfolio_isPaidAccount()) {
			return false;
		}		
